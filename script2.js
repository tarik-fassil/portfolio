ScrollReveal({ 
    reset: true ,
    distance:'60px',
    duration:2500,
    delay:350,
});
ScrollReveal().reveal('.featured-text-1', { delay: 300, origin: 'left'});
ScrollReveal().reveal('.featured-text-2', { delay: 300, origin: 'right'});
ScrollReveal().reveal('.img-about,.logo', { delay: 400, origin: 'right'});
ScrollReveal().reveal('.tex-about, .section-heading', { delay: 500, origin: 'left'});
ScrollReveal().reveal('.list', { delay: 300, origin: 'top'});
ScrollReveal().reveal('.project', { delay: 200, distance:'40px'});
ScrollReveal().reveal('.service', { delay: 300, origin: 'right'});
